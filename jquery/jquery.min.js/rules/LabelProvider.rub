...
label(?X,?label) :- (Type(?X); Field(?X); CU(?X); Package(?X); Marker(?X)), name(?X,?label).
...
methodOrField(?X,method) :- Callable(?X).
...
interfaceMethodOrField(?X):- methodOrField(?X, ?), child(?I,?X), interface(?I).
...
descriptorImg([package], DESC_OBJS_PACKDECL).
